describe('Chapter 08 Values', function() {
	...
	it('Some ways to test', function() {
		...
		/* assert.isFalse(typeof check, 'number'); Why is not working? Fix it without change de validator */
	...
	it('ECMAScript types', function() {
		...
		delete date.month;
		//assert.isFalse(??? in date);		
		date.day = 20;
		assert.isTrue('day' in date);
		date.day = undefined; //To delete an attribute, do not do this. See why ->		
		...
		//assert.strictEqual('3' * '4', ???); //Do you see the coertion?		
		//assert.strictEqual(3 + 4, ???);
		//assert.strictEqual('3' + '4', ???); //Warning!
	});

	it('Primitive values versus types', function() {
		var obj1 = {};
		var obj2 = {};
		//assert.???(obj1, obj2); //obj1 and obj2 diffrent references
		obj2 = {};
		//assert.???(obj1, obj2); //Now they are the same different references
		...
		str.length = 1;
		//assert.strictEqual(str.length === ???); //In primitive variables, properties can’t be changed, added, or removed
		str.foo = 'bar';
	...
	it('Objects', function() {
		...
		//assert.strictEqual(String(123), ???); //Carefull
		
		var bool = new Boolean(true);
		//assert.strictEqual(typeof new Boolean(true), 'boolean'); //Unwrapping Primitives
		
		assert.strictEqual('abc'.charAt, String.prototype.charAt); //Primitives don’t have their own methods and borrow them from wrappers
	...
	it('Type Coercion', function() {
		...
		//assert.is???(Boolean(false)); //Wierd, isn't it? In any way, 'false', is a falsy value
